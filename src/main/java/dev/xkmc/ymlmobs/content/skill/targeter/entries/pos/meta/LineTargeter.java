...
import dev.xkmc.ymlmobs.content.skill.targeter.types.PosToPosMetaTargeter;
...
public class LineTargeter extends PosToPosMetaTargeter {
	...
	@Override
	public Stream<LevelPosYaw> getPos(TargeterContext data, Stream<LevelPosYaw> list) {
		...
		return list.flatMap(lx -> {
			...
			if (c > 1) {
				...
				return IntStream.range(1, c).mapToObj(i -> LevelPosYaw.of(sl.level(), sv.add(v.scale(i))));
			}
			return Stream.of();
		});
	}
